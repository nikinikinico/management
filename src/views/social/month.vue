<template>
  <div class="monthStatementBox">
    ...
    <el-row type="flex" justify="center" align="middle" style="height:60px">
      <el-col :span="12">
        <el-button size="small" type="primary" @click="archivingReportForm">归档{{ yearVal ? yearVal.substr(4) : '' }}报表
        </el-button>
...
export default {
  ...
  methods: {
    transformDateFormat(row, column, date) {
      let fmt = 'yyyy-MM-dd'
    ...
    async getArchivingCont() {
      this.loading = true
      this.contentData = await getArchivingCont({
        month: this.yearVal,
        opType: 1
      })
      this.loading = false
    ...
    archivingReportForm() {
      this.$confirm(
        `您确定要归档${this.yearVal}报表？报表归档将覆盖上一次归档记录，无法恢复。`
      ).then(async () => {
