  ...
  methods: {
    ...
    async getPersonalDetail() {
      ...
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
        this.$refs.myStuffPhoto.fileList = [
    ...
    async saveUser() {
      // 去读取 员工上传的头像
      const fileList = this.$refs.stuffPhoto.fileList // 读取上传组件的数据
    ...
    // 保存基本信息
    async savePersonal() {
      const fileList = this.$refs.myStuffPhoto.fileList
      if (fileList.some((item) => !item.upload)) {
        this.$message.warning('您当前还有图片没有上传完成！')
        return
      }
